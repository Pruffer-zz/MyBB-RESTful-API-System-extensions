...
function restfulapi_install() {
	...
	$query = $db->simple_select("settinggroups", "COUNT(*) AS rows_count");
	$rows = $db->fetch_field($query, "rows_count");
	$settinggroups = array(
		"name" => "restfulapi",
	...
	$settings[4] = array(
		"name" => "apifilelocation",
		"title" => "Location for file APIs",
		"description" => "Enter a location that the APIs managing files should use. Include a trailing slash. Leave blank to deny access to the API. Example: /home/www/api_files/",
		"optionscode" => "text",
		"value" => "",
		"disporder" => "5",
		"gid" => $gid
	...
	$settings[5] = array(
		"name" => "apifileurlbase",
		"title" => "Base URL for file/url API",
		"description" => "Enter the base url that the file/url API should use. Include a trailing slash. Leave blank to deny access to the API. Example: https://example.com/api_files/",
		"optionscode" => "text",
		"value" => "",
		"disporder" => "6",
		"gid" => $gid
...
function restfulapi_uninstall() {
	...
	$db->delete_query("settings", "name IN ('enablerestfulapi', 'apikeylength', 'apirequestmethod', 'apihttpsonly', 'apifilelocation')");
				...
				if(isset($mybb->input["do"]) && in_array($mybb->input["do"], array("regenerate", "edit", "delete"))) {
					...
					elseif($do == "regenerate" && isset($mybb->input["key_id"]) && isset($mybb->input["my_post_key"]) && verify_post_check($mybb->input["my_post_key"])) {
						...
						if($db->simple_select("apikeys", "*", "id='{$key_id}'")->num_rows == 1) {
							...
							$update = array(
								"apikey" => $db->escape_string(htmlspecialchars_uni($apikey))
							);
							$db->update_query("apikeys", $update, "id='{$key_id}'");
							restfulapi_cache_rebuild();
